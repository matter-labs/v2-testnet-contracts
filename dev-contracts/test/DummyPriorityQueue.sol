...
contract DummyPriorityQueue is Base {
    ...
    function pushDummyOpToDeque(Operations.OpTree _opTree, DummyPriorityOperation memory _dummyPriorityOp) external {
        _saveDummyOperation(_dummyPriorityOp);

        s.priorityQueue[_opTree].pushBackToDeque(_dummyPriorityOp.id);
    }

    function pushDummyOpsToMainHeap(Operations.OpTree _opTree, DummyPriorityOperation[] memory _dummyPriorityOps)
        external
    {
    ...
    function pushDummyOpsToBufferHeap(Operations.OpTree _opTree, DummyPriorityOperation[] memory _dummyPriorityOps)
        external
    {
        for (uint256 i = 0; i < _dummyPriorityOps.length; ++i) {
    ...
    function getHeapWithDummyOps(Operations.OpTree _opTree)
        external
        view
        returns (DummyPriorityOperation[] memory dummyOps)
    {
        ...
        for (uint256 i = 0; i < operationIDs.length; ++i) {
            ...
            Operations.PriorityOperation memory priorityOp = s.storedOperations.inner[id];
    ...
    function getHeapBufferSize(Operations.OpTree _opTree) external view returns (uint256) {
        return s.priorityQueue[_opTree].heapBufferSize();
    }

    /// @dev removes all elements from both deques
    function clearDeques() external {
        _clearDeque(Operations.OpTree.Full);
        _clearDeque(Operations.OpTree.Rollup);
    }

    function _pushOneDummyOpsToMainHeap(Operations.OpTree _opTree, DummyPriorityOperation memory _dummyPriorityOp)
        internal
    {
        _saveDummyOperation(_dummyPriorityOp);
    ...
    function _pushOneDummyOpsToBufferHeap(Operations.OpTree _opTree, DummyPriorityOperation memory _dummyPriorityOp)
        internal
    {
        _saveDummyOperation(_dummyPriorityOp);
    ...
    function _clearDeque(Operations.OpTree _opTree) internal {
    ...
    function _saveDummyOperation(DummyPriorityOperation memory _dummyPriorityOp) internal {
        ...
        uint32 expirationBlock = _dummyPriorityOp.expirationBlock;
        uint96 layer2Tip = _dummyPriorityOp.layer2Tip;
